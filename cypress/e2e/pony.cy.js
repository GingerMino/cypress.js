describe('Проверка работоспособности оформления заказа', function () {
  ...
  return false
    ...
    it('Оформление заказа', function () {
        ...
        cy.get('.add-cart-counter__btn').wait(5000).click({ force: true})
        cy.get('[data-add-cart-counter-plus=""]').wait(5000).click({ force: true})
        cy.get('.header__cart > .icon').click()
        cy.contains('Оформить заказ')
        cy.contains('Попона "Пони-эскимони" мох')
    })
})
